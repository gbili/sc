<?php
namespace TwitterGeoProtocol\Service;
...
class Converter 
{
    ...
    {
        $text = "{$data['user']}({$data['geo_tag']}:{$data['lat']},{$data['long']},{$data['zoom']})";
    ...
    public function extractStatusesTextData(array $statuses)
    {
        ...
        foreach ($statuses as $status) {
            $status->tgp_data = $this->toArray($status->text);
        }
        return $statuses;
    }
    ...
    {
        unset($matches['score_tag'], $matches[7], $matches['score_value'], $matches[8]);
        return $matches;
    }
    ...
    {
        $status = new \StdClass();
        $status->text = 'gbili_com(majanico:28.740970,-13.936373,17)wave height:1;asdf d:23;';

        $expectedStatus = clone $status;
        $expectedStatus->tgp_data = array (
            0 => 'gbili_com(majanico:28.740970,-13.936373,17)wave height:1;asdf d:23;',
            'user' => 'gbili_com',
            1 => 'gbili_com',
            'geo_tag' => 'majanico',
            2 => 'majanico',
        ...
        if ($expectedStatus->text !== $this->toText($status->tgp_data)) {;
